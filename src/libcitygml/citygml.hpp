...
enum State {
	...
	public:
        ...
        inline const std::vector<LinearRing*>& getInteriorRings() const {return _interiorRings;}
        inline std::vector<LinearRing*>& getInteriorRings() {return _interiorRings;}

        inline const LinearRing* getExteriorRing() const {return _exteriorRing;}
        inline LinearRing* getExteriorRing() {return _exteriorRing;}
	...
	public:
        ...
        inline const std::vector< Polygon* >& getPolygons() const { return _polygons; };
        inline std::vector< Polygon* >& getPolygons() { return _polygons; };
	...
	public:
        CityObject( const std::string& id, CityObjectsType type ) : Object( id ), _type( type ), m_path("") {}
        ...
        inline std::vector< Geometry* >& getGeometries() { return _geometries; }
        inline const std::vector< Geometry* >& getGeometries() const { return _geometries; }
        ...
        const std::vector<BuildingFlag*>& getFlags() const { return m_Flags; }
        BuildingFlag* getFlag(const std::string& name)
        {
            for(size_t i=0; i<m_Flags.size(); ++i)
            {
                if(m_Flags[i]->getStringId() == name)
                    return m_Flags[i];
            }
            return NULL;
        }

        void addTag(BuildingTag* tag) { tag->m_id = m_Tags.size();  m_Tags.push_back(tag); }
        std::vector<BuildingTag*>& getTags() { return m_Tags; }
        const std::vector<BuildingTag*>& getTags() const { return m_Tags; }
	...
	protected:
        ...
        std::vector<BuildingFlag*> m_Flags;
        std::vector<BuildingTag*> m_Tags;
