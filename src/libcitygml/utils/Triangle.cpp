...
TriangleList* BuildTriangleList(std::string tilefilename, citygml::CityObjectsType objectType)
{
	...
	std::map<citygml::CityObjectsType,citygml::CityObjects>::iterator it;
	for ( it = model->getCityObjectsMap().begin();it!=model->getCityObjectsMap().end();it++)
	{
		for(citygml::CityObject* obj : it->second) //For each city object
		{
